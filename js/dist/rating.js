...
(function (global, factory) {
  ...
  class Rating extends BaseComponent {
    constructor(element, config) {
      ...
      this._name = this._config.name || index_js.getUID(`${this.constructor.NAME}-name-`).toString();
      this._tooltip = null;
      this._createRating();
    ...
    // Private
    _addEventListeners() {
      EventHandler.on(this._element, EVENT_CLICK, SELECTOR_RATING_ITEM_INPUT, ({
        target
      }) => {
      ...
      EventHandler.on(this._element, EVENT_FOCUSIN, SELECTOR_RATING_ITEM_INPUT, ({
        target
      }) => {
        EventHandler.trigger(this._element, EVENT_HOVER, {
          value: target.value
    ...
    _createRatingItem(index) {
      ...
      Array.from({
        ...
        const ratingItemId = index_js.getUID(`${this.constructor.NAME}${index}`).toString();
